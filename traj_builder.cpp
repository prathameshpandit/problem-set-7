...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
  ...
  nav_msgs::Odometry des_state;
    ...
    int vec_state=vec_of_states.size();
    double x_brake = start_pose.pose.position.x;//position when brake is applied
    double y_brake = start_pose.pose.position.y;
    double x_des = x_brake;
    double y_des = y_brake;    
    double brake_speed;
    double current_speed;
    double brake_omega;
    double current_omega;
    double psi_des=convertPlanarQuat2Psi(start_pose.pose.orientation);
    float tb=0;// braking time
    for (int i = 0 ; i < vec_state; i++){
       int delta_x = start_pose.pose.position.x - vec_of_states[i].pose.pose.position.x;
    	int delta_y = start_pose.pose.position.y - vec_of_states[i].pose.pose.position.y;
   // type of delta_x and delta_y as integer on purpose so that it can be equal to zero
    	if (delta_x<0.1 && delta_y<0.1 && delta_x>-0.1 && delta_y>-0.1 ){
    		des_state.twist.twist = vec_of_states[i].twist.twist;// current velocity and angular velocity determined
    		break;
    	}
    }
    brake_speed= des_state.twist.twist.linear.x;//speed at time of brake applied
    current_speed= brake_speed;
    brake_omega= des_state.twist.twist.angular.z;
    current_omega=brake_omega;
// used while loop as we don't know time it will take for speed to reach zero
 while(current_speed>0 && current_omega>0) { 
        tb += dt_;
        current_speed = brake_speed-accel_max_*tb;
        des_state.twist.twist.linear.x = current_speed; //update speed
        //update positions
        x_des = x_brake + brake_speed*tb - 0.5 * accel_max_ * tb * tb * cos(psi_des);
        y_des = y_brake + brake_speed*tb - 0.5 * accel_max_ * tb * tb * sin(psi_des);
	current_omega = brake_omega - alpha_max_*tb;
	des_state.twist.twist.angular.z=current_omega;
        des_state.pose.pose.position.x = x_des;
        des_state.pose.pose.position.y = y_des;
        vec_of_states.push_back(des_state);
    }
